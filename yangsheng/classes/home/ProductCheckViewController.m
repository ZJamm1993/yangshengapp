...
-(void)startChecking
{
    UIColor* color=rgb(24, 193, 67);
    NSString* la=@"√";
    NSString* imgN=@"icon_Correct";
    
    if (self.resultType!=ProductCheckResultTypeCorrect) {
        color=rgb(228, 60, 80);
        la=@"!";
        imgN=@"icon_error";
    }
    
    self.goodImage.image=[UIImage imageNamed:imgN];
    
    self.goodLabel.text=la;
    self.goodLabel.backgroundColor=color;

    self.goBackButton.backgroundColor=color;
    self.goodDescription.text=self.resultTitle;
    self.checkedTimeLabel.text=self.resultDetail;
